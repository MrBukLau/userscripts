...
function blockBannerTW(s) {
    ...
    if (mutationObserver_0 === undefined) {
        ...
        mutationObserver_0.observe(document.documentElement, {
            attributes: true
        });
    }
}
...
function quoraPatchObserver() {
    ...
    try {
        ...
        try {
            ...
            console.log("Info: if you are browsing from a PC, this error is expected.");
        }
    }
    ...
    const config = {
        ...
        childList: true,
        subtree: false
    }; //this rough implementation is slower
    const callback = function(mutationsList, observer) {
        for (const mutation of mutationsList) {
            quoraPatch(firstCheck);
        }
...
function removeAskFmLoginPopup() {
    try {
        ...
        closeButton.addEventListener("click", function(e) {
            document.getElementsByClassName("lightbox_overlay")[0].remove();
        });
